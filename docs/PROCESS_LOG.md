...
## 5. auto 모드에서도 뜨던 확인창 — 가드 훅이 자동 진행을 막고 있었다 (2026-09-21)

정본: [../AGENTS.md](../AGENTS.md) §2.1·§2.2, [../scripts/claude-scope-guard.mjs](../scripts/claude-scope-guard.mjs), [../scripts/claude-git-guard.mjs](../scripts/claude-git-guard.mjs), [DECISIONS.md](DECISIONS.md) D-060.

### 5.1 무슨 일이 있었나

사용자가 권한 모드를 auto로 해 두었는데도 여러 세션에서 "Claude가 편집 …하도록 허용하시겠습니까?" 창이 계속 떴다. 한 세션의 스크린샷에서는 세션 scratchpad의 `flow.mjs` 편집에 창이 떴고, 본문 첫머리가 `'../../../NOHTAE~1/AppData/Local/Temp/claude/…'는`으로 시작했다 — 소유 파일 가드가 만드는 문구였다.

### 5.2 원인

1. **훅의 `ask`는 auto 모드도 막지 못한다.** 공식 문서(권한 모드)가 훅이 강제한 프롬프트는 auto에서도 표시된다고 적는다. 두 가드는 "차단이 아니라 확인"이라는 설계(D-042, §2.5)라 사용자가 자동 진행을 택한 세션에서도 그대로 창을 띄웠고, 훅이 낸 창에는 "이 세션 동안 허용"이 없어 매번 반복됐다.
2. **소유 파일 가드가 Claude Code 자신의 작업 폴더를 저장소 밖이라는 이유로 매번 확인 대상으로 삼았다.** 세션 기록에 남은 훅 확인 94건은 scratchpad 47·자동 메모리 폴더 30·기타 프로젝트 밖 1·git 16이고, 프로젝트 안 파일에 대한 확인은 0건이었다(9/19~9/21에 몰림).
3. **가드가 죽어 있었다.** CURRENT_TASK 선언 줄의 설명문 안에 backtick으로 싼 `?theme=pebble`(URL 조각)이 허용 목록 패턴으로 들어갔고, `?`를 이스케이프하지 않아 정규식 오류로 훅이 29회 죽었다. Claude Code는 훅이 exit 1로 끝나면 비차단 오류로 보고 도구 호출을 그대로 진행하므로 "hook error" 알림만 남고 소유 밖 프로젝트 파일 검사가 통째로 꺼져 있었다. 테스트는 fixture로만 돌아 이 토큰을 만나지 못했고, 그 테스트도 `--mode full`(`npm test`)에서만 돌았다 — 매 턴 도는 quick·Stop 훅은 가드 파일이 있는지만 봤다.

### 5.3 조치

D-060. 훅이 입력의 `permission_mode`가 auto이면 아무것도 출력하지 않는다. 하위 에이전트(입력에 `agent_id`)의 커밋·푸시만 auto에서도 확인한다. 세션 scratchpad(`scratchpad_dir`)와 자동 메모리 폴더(`transcript_path`가 놓인 `projects/<프로젝트>` 아래 `memory`)는 어느 모드에서나 제외한다. 두 폴더는 Windows에서 `NOHTAE~1` 같은 짧은 이름과 긴 이름이 섞여 나오므로 실경로로 맞춰 비교한다. `?`는 글자로 취급하고 정규식이 되지 못하는 토큰은 건너뛴다. 모드를 알 수 없으면 종전대로 확인한다.

### 5.4 검증

- **실측**: 계획 승인 뒤 가드에 임시 프로브를 넣어 이 앱(Claude Code 2.1.275)의 PreToolUse 입력을 기록했다 — `permission_mode: "auto"`, `scratchpad_dir`(짧은 이름 표기), `transcript_path`(긴 이름 표기)가 오고 하위 에이전트 호출에는 `agent_id`·`agent_type`이 붙었으며 하위 에이전트도 `auto`를 이어받았다. 프로브는 확인 뒤 지웠다.
- **파이프 테스트**: 소유 파일 가드 30건·git 가드 25건 통과. 수정 전 가드(HEAD)에 새 테스트를 돌리면 16건·5건이 실패한다 — `?` 토큰, 모드, 작업 폴더, 훅 종료 코드, 실제 CURRENT_TASK 스모크가 각각 옛 결함을 잡는다.
- **과거 호출 재적용**: 세션 기록 16개의 Write·Edit 727건과 Bash 888건을 새 가드에 넣었다(모드 auto) — 확인 0건, 훅 오류 0건. 같은 호출을 default로 넣으면 scratchpad·메모리는 여전히 0건이고, 프로젝트 안 소유 밖 파일 218건·기타 프로젝트 밖 3건과 git 52건이 확인 대상으로 남는다(설계대로). 하위 에이전트 호출은 주 기록에 없어 재적용하지 못했다.

### 5.5 배운 것

- **만들 때 상호작용을 확인하지 않은 안전장치는 사용자에게 소음이 된다.** "확인한다"는 안전한 기본값처럼 보이지만 자동 진행을 택한 사용자에게는 호출마다 방해다. 훅이 모드를 볼 수 있다는 것(입력의 `permission_mode`)을 처음 만들 때 확인했어야 한다.
- **죽어도 조용한(fail-open) 훅은 죽었다는 사실을 따로 검사해야 한다.** 이 훅은 "훅 오류로 작업을 막지 않는다"는 원칙이라 크래시가 곧 검사 해제였고, 3일간 아무 데서도 잡히지 않았다. 테스트가 종료 코드를 보지 않고 fixture만 썼다 — 이제 종료 코드를 보고, 실제 선언 줄로 한 번 돌려 보는 스모크를 넣었다.
- **선언 줄 설명문에 backtick으로 싼 조각이 들어가면 파서가 그것을 경로로 읽는다.** 이제 훅이 죽지는 않지만 선언 줄에는 경로·글롭만 backtick으로 싸고 설명은 backtick 없이 쓰는 것이 낫다(AGENTS §2.2에 적었다).

### 5.6 남은 것

- auto 모드에서는 소유 파일 검사가 꺼진다. 소유 파일 밖 출처 불명 쓰기의 사후 점검(이슈 #27이 제안한 세션 경계 스냅샷)은 여전히 없다.
- `bypassPermissions`·`acceptEdits` 모드는 종전대로 확인한다. `bypassPermissions`를 쓰게 되면 같은 처리를 할지 정한다.
- 앱 자체가 띄우는 창(브라우저 사이트 허용 등)은 훅과 무관해 이 변경으로 없어지지 않는다.
- Codex 세션에는 이 훅이 없다(§2.3).

## 6. 여러 작업자로 시안을 만들며 사용자 정정을 세 번 받았다 (2026-09-22)

정본: [DECISIONS.md](DECISIONS.md) D-066~D-072, [../tasks/CURRENT_TASK.md](../tasks/CURRENT_TASK.md) QA 처리 기록 26~30.

### 6.1 무슨 일이 있었나

사용자가 하루 동안 요청한 시안 수정(마음 고르기·오늘 화면·세부 감정 바구니·크기·이유·오늘 있었던 일·칭찬·감사·편지·달력·통계)을 Sonnet 작업자 여럿이 소유 파일을 나눠 병렬로 만들었다. 통합 검토에서 사용자 정정이 세 번 있었다. ① 오늘 화면의 조약돌을 계열마다 셋(27개)으로 만든 것 — 사용자는 "이전처럼 계열당 하나"였다. ② 달력·통계·설정을 오늘 화면과 같은 어두운 숲+유리 탐색으로 만든 것(작업자 시안) — 사용자는 "나머지 화면은 배경이 밝은 색, 네비게이션도 원래 것"이라며 흰 바탕 통계 시안을 첨부했다. ③ 편지 배경 — 처음 밤의 책상, 다시 '밝은 낮'으로 지시하자 황토빛 나무 책상이 나왔는데 화면 절반이 어두워 옅은 민트 책상으로 고쳤다.

### 6.2 원인

1. **브리프가 사용자의 시각 제약을 원문으로 못 박지 않았다.** "오늘 화면과 같은 무드로"라고 요약하자 작업자는 색·탐색 구조까지 확장해 읽었다. '흩어진 조약돌'도 내가 계열당 셋으로 해석해 브리프에 적었다 — 개수를 사용자가 이미 정해 둔 것(이전과 같은 아홉)을 확인하지 않았다.
2. **작업자의 자체 보고는 국소 최적화였다.** 편지 작업자는 봉투가 책상에 묻힌다는 휘도 대비만 보고 책상을 진하게 했다. 색상 차(CIEDE2000)로 재면 옅은 민트 책상에서도 봉투가 충분히 갈렸다(19.4~19.8, 흰 바탕 위 19.2, 황토 책상 16.4).
3. **손으로 계산한 대비를 문서에 먼저 적었다.** 초록 하늘 위 danger 문구를 6.49:1로 적었으나 실제는 4.38:1이었다(D-066에서 정정).
4. **겹침(음수 여백)으로 만든 배치가 실제 높이를 잘못 셌다.** 숲 위에 겹치는 하단 탐색이 유리 윤곽 2줄과 안전 영역을 빼고 있어 오늘 화면이 2px(iPhone 안전 영역에서는 36px) 스크롤됐고, 길의 아래 모서리가 유리 알약 밖으로 삐져나왔다. 화면 순회 중 `scrollHeight`를 재고서야 잡혔다.

### 6.3 조치

- 오늘 화면만 어둡고 나머지는 밝다는 것을 D-070으로 못 박고 밝은 탭과 낮의 편지(D-069)로 되돌렸다.
- **화면 순회를 통합 검토의 기본 절차로 삼았다**: 전 경로와 작성 흐름을 클릭으로 끝까지 진행하며 두 크기(390×844·360×740)에서 가로·세로 넘침, 깨진 그림, 콘솔 오류를 잰다. 문서 정정은 소유 파일을 나눈 문서 작업자에게, 코드 작업자가 보고한 죽은 CSS 규칙은 CSS 정리 작업자에게 맡겼고 정리는 지우기 전후 화면의 픽셀 차이 0으로 증명하게 했다.
- `check-contrast`에 편지 책상 등 검사를 더해(331건) 수치가 문서가 아니라 스크립트에 있게 했다.

### 6.4 배운 것

- **사용자의 시각 제약은 브리프 첫머리에 원문으로 인용한다**("나머지 화면은 배경이 밝은 색", "네비게이션은 원래 것"). 요약해 "메인 무드에 맞춰"라고 쓰면 색과 구조까지 번진다. 사용자가 이미 정한 개수·이름도 코드에서 확인해 적는다.
- **작업자에게 "무엇이 묻히나"를 재게 할 때는 휘도 대비만이 아니라 색상 차까지 재게 한다.** 휘도만 보면 항상 더 진한 쪽으로 밀린다.
- **겹침으로 만든 배치는 실제 높이를 식으로 계산하고 `scrollHeight`로 확인한다.**
- **손으로 계산한 대비는 문서에 쓰지 않는다.** 스크립트로 재고 검사에 넣은 뒤에 쓴다.

### 6.5 남은 것

- 마음 고르기(D-066)는 사용자 확인을 기다리는 provisional이다.
- 실기기(iPhone Safari·Android Chrome), OS의 움직임 줄이기, Firefox의 4px 스크롤 막대는 확인하지 못했다.
- 360×640에서는 편지 묶음이 89px, 오늘 있었던 일·칭찬·감사 화면이 95px 스크롤된다(알려진 한계, D-069).

## 7. 통계·설정 재구성부터 온보딩 이야기·위기 안내까지 — 하루 여덟 결정 (2026-09-25)

정본: [DECISIONS.md](DECISIONS.md) D-090~D-096, [../tasks/CURRENT_TASK.md](../tasks/CURRENT_TASK.md) QA #50~#54와 2026-09-25 체크포인트·인수인계.

### 7.1 무슨 일이 있었나

- **통계·설정(D-091)**: 사용자가 통계의 UI·UX 전반 분석과 하단 탐색 뒤 직사각형 잘림, 글 위주 설정을 지적했다. 2배 캡처 분석(통계 8건·설정 4건)과 이전 IA(추가 화면 G2~G8) 대조 뒤 질문 세 라운드로 방향을 정하고 Sonnet 작업자 셋(통계·설정 목록·내보내기/삭제)을 병렬로 돌렸다. 토스의 짜임만 빌리고 색·부품은 빌리지 않았다(충돌하면 우리 디자인 시스템 우선).
- **온보딩 장면 이야기(D-092)**: '#/welcome을 WHY를 말하는 모션그래픽으로' 요청에 영상·Lottie 대신 앱 안 WAAPI 장면을 골랐다. 별도 서버(4180)의 초안으로 열 번 넘게 사용자 검토를 받아 5장면 → 6장면, 1장면 옅은 민트, 길은 아래에서부터, 섞인 감정(홀가분+허전), 4장면 조약돌 빼기·나의 돌 키우기, 줄 끝 쉼표 빼기, 고지 쪽 길로 잇기를 정한 뒤 앱으로 옮겼다. 로딩 화면은 스크립트보다 먼저 그려야 해 숲을 `web/splash.svg`로 굳히고 verify가 어긋남을 잡게 했다. 마감 검토의 WCAG 2.2.2 지적으로 일시정지를 더했다.
- **온보딩 안내 정리(D-093)**: 다른 세션의 법적 근거 조사(`research/legal-onboarding-crisis-2026-09.md`)를 사용자가 붙여 A/B를 물었다 — 안 A(온보딩 안전 안내·위기 링크 제거, 위기 안내는 설정 입구만), '시작하기 전에'는 WHY 약속 두 줄, 건강 민감정보 별도 동의는 '받는다', AI 사전 고지는 할 일로.
- **위기 안내(D-094)**: '아래에서 지금 바로 도움을…'인데 연락처가 없던 화면을 읽기 전용 조사자가 공식 누리집으로 확인한 번호로 채우고, 빨간 상자·상자 속 상자·큰 '돌아가기'를 걷어 '연락하기'가 주 동작이 되게 했다. 번호는 versioned resource 파일 한 곳에 두고 verify가 허용 번호·공식 출처를 검사한다.
- **통계 친구 상세(D-095)·달력 편지(D-096)**: '각 감정 안에 들어갔을 때 AI가 디자인한 것 같다'는 지적에 흔한 분석 앱 틀을 우리 언어(언덕 위 친구·열 점 기둥·감정 chip·조약돌 줄)로 바꿨고, '처음부터 크게, 옆에 살짝 보이는 것 없애, 배경에 파묻힌 느낌' 지적에 달력에서 곧장 큰 편지·엿보기 제거·종이 카드 그림자(원칙의 두 번째 예외)를 정했다.

### 7.2 원인과 발견

1. **작업자 보고 통과가 곧 완성은 아니었다.** 통계 머리 장면(직사각형으로 잘린 언덕·바위처럼 읽힌 회색 반원), 편지 카드의 '파묻힌' 원인(옆 카드 엿보기용 좌우 흐림 마스크가 엿보기를 없앤 뒤 카드 가장자리를 흐림), 달력 카드 높이 계산(아래 ‹ 점 › 줄·흐림 띠를 빼지 않아 달 제목이 띠에 반쯤 걸림)은 모두 메인이 새 프로필 캡처로 다시 재서 찾았다.
2. **새 맥락 검토자가 코드 경로의 빈틈을 찾았다.** 로딩 화면이 시작 실패 때 오류 문구를 덮음, 온보딩에서 뺀 건강 정보 안내가 앱 어디에도 없음, 고지가 열려도 뒤 버튼에 초점, 1388이 휴대전화에서 지역번호가 필요함, 112·119가 파일을 기다리는 동안 없음, 열 점의 채운 점·빈 점 대비 부족, 달력 리스너 누적 — 캡처만으로는 보이지 않는 것들이다.
3. **'크게 보기'를 없애자 완료 취소·삭제로 가는 길도 사라졌다.** 문서 동기화 작업자가 코드를 읽다 찾았다 — 화면 하나를 줄일 때 그 화면만 가진 기능이 무엇인지 먼저 세어야 한다.
4. **도구 함정 둘**: 캡처 도구가 같은 브라우저 프로필을 다시 쓰면 모듈이 HTTP 캐시에 남아 옛 화면을 찍었다(프로필 이름을 바꿔 피함). 크롬에서 `container-type:size` 안의 절대 배치 SVG에 `100cqh`가 0으로 풀려 그림이 사라졌다(%와 `max-height:cqw`로 대체).

### 7.3 배운 것

- 작업자 결과는 **새 프로필·확대 캡처로 메인이 다시 잰다**. '박스 속 박스·잘린 가장자리·흐린 가장자리·반쯤 걸린 글자'를 먼저 본다.
- 마감 검토는 **새 맥락 검토자**에게 맡기고 결과는 심각도와 무관하게 모두 반영하거나 사유를 남긴다(세 차례: 온보딩 9건, D-094~096 13건 — 모두 반영).
- 법·안전이 걸린 문구는 **사실 조사(공식 출처)와 제품 결정을 분리**한다. 법적 의무가 없다는 것과 두지 않는다는 것은 다른 결정이고, 남는 의무(효능 표시 금지·민감정보·자살유발정보·AI 고지)는 문구가 아니라 설계·처리방침·입구에서 진다.
- 초안은 앱 밖(별도 서버)에서 빠르게 돌려 사용자와 합의한 뒤 옮기면 앱 쪽 재작업이 줄었다.

### 7.4 남은 것

- 출시 차단: 위기 안내 연락처 임상·안전 검토, 휴대전화 1388 통신사 실측, 건강 민감정보 별도 동의 흐름, AI 사전 고지 한 줄(AI 작성이 열릴 때).
- 실기기(iPhone Safari·Android Chrome)·실제 OS 움직임 줄이기·Firefox·Safari에서 온보딩·로딩 화면·달력 편지를 보지 못했다.
- 통계 친구 상세의 기쁨 친구(나래)는 원본이 뛰어오른 자세라 발이 길에서 살짝 떠 보인다. 머리 장면의 길은 크기 화면(slider.js)의 길을 재사용하지 않은 간이 곡선이다.
- 달력 편지는 360×640에서 ‹ 점 › 줄까지 보려면 손으로 조금 올려야 한다(카드 최소 376px, D-079).
