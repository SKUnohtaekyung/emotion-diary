...
| D1의 조건부 UPDATE/CHECK/trigger/batch가 데이터 계약(D-024) 충족, 무료 한도·미사용 정지 없음 | B-04 | unknown | 무료 범위의 대체 DB 재평가 |
| 서버 접근 토큰 cookie가 요구 인증 경계 충족(토큰 없는 접속 거부, rate limit) | B-02 | unknown | Cloudflare Access 추가 또는 무료 private hosting |
| Workers secret이 client bundle/로그/`wrangler.toml`에 없고 custom header CSRF 통과, worker secret 경로 분리 | B-03 | unknown | 배포 중단 |
| PC worker가 `ai_jobs` lease→`claude -p`→결과 반환 왕복을 완료하고, PC 꺼짐 시 job 만료+직접 작성 fallback | B-06 | unknown (모델 호출 자체는 partial) | AI 대화·RAG 보류 |
