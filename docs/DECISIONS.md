...
| D-030 | provisional | hosting 후보를 ChatGPT Sites 대신 **본인 PC 자체 호스팅**(Node 서버+SQLite, 휴대폰은 Tailscale 등 무료 사설망으로 접속)으로 교체한다 | D-029의 `claude -p`는 본인 PC에서만 호출 가능하므로 서버가 같은 PC에 있어야 한다. 부수 효과로 Sites/D1 의존이 사라지고 일기 데이터가 본인 PC를 벗어나지 않아 B-02/B-05 경계가 단순해진다. PC가 켜져 있어야 한다는 가용성 제약이 있다. **PRD PR-002의 "Sites 후보" 문구 변경은 사용자 승인 필요** — 승인 전까지 provisional | 사용자 승인 시 accepted, PR-002·ARCHITECTURE §2/§6·EVAL B-02/B-04/B-08 재정의 |
