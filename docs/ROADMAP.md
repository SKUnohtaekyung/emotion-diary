...
- G2c `model-access-spike`(RAG 트랙, critical path 밖): 유료 API 없이 구독 기반 Codex/Claude 자원으로 server-only 호출·strict schema·보존 통제·독립 verifier 호출이 가능한지(B-06 재정의, D-019)
