...
- G3b `taxonomy-v2-research` 세부 감정 목록의 심리학 문헌 리서치(D-038). 목적은 **제품이 쓸 목록 확정**이며 게이트는 `taxonomy-evidence`(문헌 축 충족·반증 표·근거 등급·라이선스·검수). G3a와 **병렬 가능하고 G3a를 기다리지 않는다** — 다만 항목 귀속 단계는 v1 전사본을 입력으로 쓰므로 그 시점까지 G3a가 끝나야 한다. 공포·혐오 계열이 신설되어 캐릭터 자산이 7종→9종이 되므로 자산 생성은 이 항목 뒤에 온다(자산은 2026-09-19 이슈 #26으로 완료). **완료(2026-09-04~05)**: taxonomy v2 9계열 194개 확정(`data/taxonomy/v2.json`), 반증 표 판정·근거 등급·라이선스 판정 완료(D-038 `accepted`, D-040), 독립 검증(T7a) 8/8 통과, 사용자 최종 대조(D-027) 완료로 `review_status: reviewed`. 게이트 `taxonomy-evidence`의 라벨 5개(`literature-axes-covered`·`falsification-table-complete`·`evidence-grade-recorded`·`license-check-passed`·`user-or-human-review`)를 전부 충족했고 `harness/work-graph.yaml`의 `taxonomy-v2-research` 노드는 `done`이다.
...
- 원형 6영역 직접 작성
- 확정된 taxonomy v2(9계열 194개, Phase 0의 G3a·G3b에서 완료)를 seed로 적재
- 복수 세부 감정과 각 1~10 강도
...
- 최소 50개 합성 대화 평가
- AI 사용 사전 고지 한 줄을 AI 작성 입구에 둔다(인공지능 기본법 제31조 제1항, 시행령 제23조 — 약관 기재 또는 화면 표시). AI 결과물에는 AI가 만들었다는 표시를 함께 둔다(제31조 제2항). 온보딩일 필요는 없다 — 이 단계에서 AI 진입점이 열릴 때 같이 만든다(D-093, `SAFETY_POLICY` §10)
...
- AI 진입점은 비활성 또는 미제공으로 정확히 표시하고, 위기 연락처는 설정 입구의 위기 안내 화면(`#/help`)에서 `data/crisis-resources/kr.json`으로 제공(D-020·D-094)
- **출시 차단**: 위기 안내 연락처의 임상·안전 검토와 표시 직전 재확인 — 번호·운영 시간·순서·문구, 휴대전화에서 1388이 지역번호 없이 걸리는지(공식 안내는 '지역번호+1388', 통신사 실측 필요), `kr.json`의 `review.clinicalSafety`를 `pending`에서 바꾸는 근거를 남긴다(D-094)
- 알려진 제한 공개: AI 대화·분석 미제공, 직접 작성 원문 자동 감시 없음
- 건강 민감정보 별도 동의 흐름을 이 릴리스 전에 설계·구현한다(개인정보 보호법 제23조 — 별도 동의를 받는 방향으로 정했다, D-093, `SAFETY_POLICY` §3). 동의 문구·시점·거부 시 동작을 처리방침 제30조와 함께 정한다
