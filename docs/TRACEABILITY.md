...
| PR-010 색상·캐릭터 | `UX_SPEC` §8, §12; `DESIGN_SYSTEM` §3, §8, §9; `design/tokens.json`; `design/characters/prompts.json`; source images | EVAL §6 source fidelity, 색 외 의미, AA/스크린리더, `scripts/check-contrast.mjs`(대비+계열 ΔE), `scripts/check-characters.mjs`(자산 규격) | taxonomy-v1, direct-journal |
