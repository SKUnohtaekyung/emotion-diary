...
| PR-004 7개·세부 감정 원자료 | `DATA_MODEL` §4, §3.2(D-022 code 규칙); `UX_SPEC` §8; `references/README`; `data/taxonomy/v1.json`(2026-09-04 전사·사용자 검수 완료, `review_status: reviewed`, D-027 §7.8b); `data/taxonomy/v2.json`(2026-09-05, 9계열 194개, ~~`review_status: pending_user_review`~~ **[정정, 2026-09-05]** `review_status: reviewed`, 사용자 최종 대조 2026-09-05 완료); D-038(`accepted` — ~~사용자 최종 대조 D-027 대기~~ **[정정]** D-027 완료로 완결, 반증 표 미해소 지적을 인지한 승인 — `taxonomy-v2-decisions.md` §2) | EVAL §6 모든 label/category 1:1 대조+D-027 검수; v2는 항목별 출처·근거·검수자 기록(완료, 독립 검증 T7a 8/8 통과) | taxonomy-v1, taxonomy-v2-research |
| PR-005 복수선택·강도 | `DATA_MODEL` §3.2(CHECK); `UX_SPEC` §4(검색/필터); `DESIGN_SYSTEM` §6.2(별자리 지도)·§6.3(소프트 리스트, 카테고리 그룹화)·§6.4(슬라이더), D-037·D-041 | F-05, 범위/중복 경계 fixture, 모바일/스크린리더, 슬라이더 키보드 조작, 카테고리 다중 선택 접근성(`aria-pressed`/`aria-live`) | taxonomy-v1, direct-journal |
...
| PR-010 색상·캐릭터 | `UX_SPEC` §8, §12; `DESIGN_SYSTEM` §3, §8, §9; D-044·D-045·D-046; `design/tokens.json`; `design/characters/prompts.json`; `design/characters/README.md`; `mood-preview-v2.png`; source images | EVAL §6 source fidelity, 색 외 의미, AA/스크린리더, 대표·UI 정적 보조 포즈의 40px·120px 판독과 투명 halo, 2초 loop·중심 고정, lossless animated WebP 선택과 APNG 비교 이력, iPhone Safari·Android Chrome, reduced-motion 정적 대체, `scripts/check-contrast.mjs`, `scripts/check-characters.mjs` | taxonomy-v1, direct-journal |
