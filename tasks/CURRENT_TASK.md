...
- 다음: 사용자가 `npx wrangler login`(브라우저 승인)을 마치면 **원격 G2a**: `wrangler d1 create` → migration 적용 → Workers 배포(secret은 `wrangler secret put`) → 휴대폰 접속·B-08 → rate limit·Cloudflare Access 검토. 이어서 G2c 실제 worker 프로세스+`claude -p` 결합. G3 taxonomy-v1은 병렬 가능. G3 taxonomy-v1 전사(D-022 code, D-027 검수)는 G2와 병렬 가능. G2b는 G2a 통과 후, G2c/G2d는 RAG 트랙으로 critical path 밖.
