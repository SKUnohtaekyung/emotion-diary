...
`done(결정)` — 2026-09-02 사용자 요청("디자인 시스템 잡아야 할 것 같다", "새 세션에서 대화하며 정하고, 프리뷰로 계속 확인하고 싶다"). 2026-09-04 세션에서 체크리스트 10건을 모두 확정했다 — D-034(감정 색, 글꼴, 다크 모드), D-035(chip 선택 표시, 카테고리 토글·캐릭터, 하단 탐색 이름), D-036(강도 선택기, 앵커 문구, 위기 톤, 브랜드, 탐색 아이콘, 밀도). **D-033은 accepted로 전환했다.** 남은 것은 결정이 아니라 실행이다 — 캐릭터 자산 7종 생성과 실기기 확인.
...
- [x] chip 규칙과 선택 표시 — **결정(D-035)**: 현재 안 유지(fill 100 / border 300 / text 900, 선택 시 2px 500 + 체크). 체크라는 형태 단서가 이미 있어 색만으로 구분하지 않는다.
- [x] 카테고리 토글과 캐릭터 아이콘 — **결정(D-035)**: 2줄 격자로 7개를 한 화면에(칸 80×88, 아이콘 40px). 캐릭터는 원자료와 같은 손그림 수채 무드로 codex imagegen 생성, 투명 PNG(원본 1024 / 배포 120). 프롬프트·규격·검수는 `design/characters/`. **자산 생성은 아직 안 했다 — taxonomy v1 검수와 함께 진행.**
- [x] 강도 선택기 — **결정(D-036)**: 5개씩 2줄 세그먼트(360px에서 57.6×44). 앵커 문구는 한 줄만 — 선택 전 `1 거의 스쳐 지나감 → 10 일상에 큰 영향`, 선택 후 구간 문구(1~3 / 4~7 / 8~10).
...
- 다음(실행): ① 캐릭터 7종 생성(codex imagegen, `design/characters/prompts.json`) → `node scripts/check-characters.mjs` 통과 → README §3 눈 검수. ② 실기기(iPhone Safari·Android Chrome)에서 chip 대비, 40px 아이콘 판독성, 44px 터치 확인. ③ 위기 안내 **연락처 값** 검수. 세 가지 모두 결정이 아니라 실행·검증이다.
