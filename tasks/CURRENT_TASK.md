...
- 다음(실행·리서치):
  ...
  2. **캐릭터 7종 생성**(codex imagegen, `design/characters/prompts.json`) → `node scripts/check-characters.mjs` 통과 → README §3 눈 검수.
  3. **taxonomy v2 심리학 리서치(D-038)** — G3 원자료 전사(v1) 후 진행. 카테고리 구성 변경은 사용자 승인 사항이며, 바뀌면 계열 색(ΔE·대비)과 `emotion_code` 마이그레이션이 함께 움직인다.
