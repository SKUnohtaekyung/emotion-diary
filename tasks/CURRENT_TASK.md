...
- 다음: 사용자가 D-030(자체 호스팅)을 승인하면 **G2a**를 로컬 서버 기준으로 재정의해 착수. 미승인이면 기존 Sites 기준 G2a(프로젝트 생성 전 재확인). G2c 잔여 검증(보존 설정, verifier 2회 호출, 지연 p95 10회, 위기 케이스)은 G2a와 병렬 가능. G3 taxonomy-v1 전사(D-022 code, D-027 검수)는 G2와 병렬 가능. G2b는 G2a 통과 후, G2c/G2d는 RAG 트랙으로 critical path 밖.
