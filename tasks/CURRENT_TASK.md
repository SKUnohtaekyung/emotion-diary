...
- 다음: **G2a** `hosting-identity-spike`(B-02/B-03/B-08): Cloudflare 계정·Pages/Workers/D1 프로젝트 생성은 무료지만 외부 자원이므로 실행 직전 사용자 재확인 → 최소 Workers 배포 → 접근 토큰 cookie·rate limit·CSRF·worker Bearer 경로 검증 → 휴대폰 접속. G2b `data-store-spike`는 G2a 통과 후. G2c 잔여(큐 왕복, 보존 설정, verifier 2회 호출, 지연 p95 10회, 위기 케이스)와 G3 taxonomy-v1은 병렬 가능. G3 taxonomy-v1 전사(D-022 code, D-027 검수)는 G2와 병렬 가능. G2b는 G2a 통과 후, G2c/G2d는 RAG 트랙으로 critical path 밖.
