> **아카이브(2026-09-20, TASK-INFRA-01).** `tasks/CURRENT_TASK.md`에서 옮긴 기록이다. 옮기기 직전 커밋은 `b54a0a2`이고 아래 본문은 옮기기 전과 바이트 그대로다. **상태:** done(2026-09-06) — 훅은 동작 중이고 이슈 #27은 아직 열려 있다(닫기는 사용자 결정). 알려진 한계는 AGENTS §2.2. 이 파일의 `- 소유 파일:` 선언은 scope-guard에 더 이상 반영되지 않는다 — 훅은 `CURRENT_TASK.md`만 읽는다.
