...
## 현재 배포 전제 (2026-09-02, D-029/D-030)

- 제작자 본인 PC에서 Node 서버와 SQLite 파일로 자체 호스팅하고, 휴대폰은 Tailscale 같은 무료 사설망으로만 접속하는 단일 사용자 MVP다. 공개·다중 사용자 전환은 별도 결정과 보안 검증 없이는 금지한다.
- AI 기능은 유료 API를 쓰지 않고 본인 PC의 Claude 구독 로그인으로 `claude -p`를 호출하는 경로만 사용한다. 본인 1명 전용이며 타인에게 제공하지 않는다.
...
- 실제 코드 구조는 로컬 서버·SQLite·사설망 접속 스파이크(G2a/G2b)를 통과한 뒤 생성한다. 후보가 부적합하면 `ARCHITECTURE`와 `DECISIONS`를 근거와 함께 고쳐 무료 범위의 대체 hosting/backend를 선택한다.
