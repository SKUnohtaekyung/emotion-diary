...
- ChatGPT Sites의 제한 공유와 D1을 사용하는 단일 사용자 MVP를 우선 검증한다.
- Sites 접근 제어가 MVP 로그인 경계로 충분한지는 부트스트랩에서 확인한다. 공개·다중 사용자 전환은 별도 결정과 보안 검증 없이는 금지한다.
- 개인 일기 원문은 심리학 Evidence Vector Store와 분리한다.
- 의료 기록·보호대상 건강정보(PHI)를 처리하는 제품이 아니다.
- `.openai/hosting.json`과 실제 코드 구조는 Sites 프로젝트를 생성하고 런타임 스파이크를 통과한 뒤 생성한다. 프로젝트 ID나 바인딩 이름을 추측해 커밋하지 않는다. 후보가 부적합하면 `ARCHITECTURE`와 `DECISIONS`를 근거와 함께 고쳐 대체 hosting/backend를 선택한다.
